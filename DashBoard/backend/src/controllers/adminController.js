...
const createUser = async (req, res) => {
  try {
    ...
    const [existingUser] = await pool.execute(
      'SELECT id, username, bc_number FROM users WHERE username = ? OR bc_number = ?',
      [username, bc_number]
    );
    ...
    if (existingUser.length > 0) {
      ...
      if (existingUser[0].bc_number === bc_number) {
        return res.status(400).json({ error: 'BC number already exists' });
      }
      return res.status(400).json({ error: 'Username or BC number already exists' });
    }
