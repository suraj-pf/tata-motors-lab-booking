...
const createBooking = async (req, res) => {
  ...
  try {
    ...
    // Check for pending bookings in queue
    ...
    // If there are pending bookings, this booking goes to queue
    if (pendingBookings.length > 0) {
      bookingStatus = 'pending';
      queuePosition = pendingBookings.length + 1;
    }
    ...
    // Get lab details for socket emissions
    const [labDetails] = await pool.execute(
      'SELECT name, building FROM labs WHERE id = ?',
      [lab_id]
    );
    const labName = labDetails[0]?.name || 'Lab';

    // Emit socket events based on booking status
    if (bookingStatus === 'confirmed') {
      // Emit booking created event to lab room
      ...
      // Emit room status update for immediate UI refresh
      emitToLab(lab_id, 'room-status-update', {
        labId: lab_id,
        status: 'booked',
        bookingId: result.insertId,
        bookingDate: booking_date,
      ...
      emitToUser(req.user.id, 'booking-confirmed', {
        ...bookingData,
        message: `Booking confirmed for ${labName}`,
        timestamp: new Date().toISOString()
      });
      ...
      emitToAdmins('admin-notification', {
        ...
        message: `New booking for ${labName} by ${req.user.name}`,
        booking: bookingData,
        timestamp: new Date().toISOString()
      });
    } else {
      // Pending booking notification
      emitToUser(req.user.id, 'booking-pending', {
        ...bookingData,
        message: `Booking added to queue for ${labName}. Position: ${queuePosition}`,
        timestamp: new Date().toISOString()
      });

      emitToAdmins('admin-notification', {
        type: 'booking-pending',
        message: `New pending booking for ${labName} by ${req.user.name}`,
        booking: bookingData,
        timestamp: new Date().toISOString()
      });
    }
...
const cancelBooking = async (req, res) => {
  ...
  try {
    ...
    // Notify admins if cancelled by admin
    ...
    // If this was a confirmed booking that was cancelled, process next in queue
    if (booking[0].status === 'confirmed') {
      await processQueue(booking[0].lab_id, booking[0].booking_date, booking[0].start_time, booking[0].end_time);
    }
...
const processQueue = async (labId, bookingDate, startTime, endTime) => {
  const connection = await pool.getConnection();
  try {
    // Find the first pending booking for this time slot
    ...
    if (nextInQueue.length > 0) {
      // Auto-approve the next booking in queue
      await connection.execute(
      ...
      if (bookingDetails.length > 0) {
        emitToUser(bookingDetails[0].user_id, 'booking-auto-approved', {
          ...bookingDetails[0],
          message: `Your booking for ${bookingDetails[0].lab_name} has been auto-approved from the queue!`,
          timestamp: new Date().toISOString()
        });
        ...
        emitToLab(labId, 'room-status-update', {
          ...
          startTime: bookingDetails[0].start_time,
          endTime: bookingDetails[0].end_time,
...
const getUserBookings = async (req, res) => {
  try {
    ...
    // Simple, reliable query to get user's bookings
    const [bookings] = await pool.execute(
      `SELECT 
        b.id,
        b.lab_id,
